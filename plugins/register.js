...
let handler = async function (m, { conn, text, usedPrefix }) {
  ...
  if (user.registered === true) throw `Anda Sudah *TERDAFTAR*\n• Mau Daftar Ulang? ${usedPrefix}unreg <SN|SERIAL NUMBER>\n• Contoh: #unreg H85fI74gJw85G4Jo5U\n\nLupa SN? Cek Di #ceksn`
  if (!Reg.test(text)) throw `Format salah\n*${usedPrefix}daftar nama.umur*`
  let [_, name, splitter, age] = text.match(Reg)
  if (!name) throw 'Nama tidak boleh kosong (Alphanumeric)'
  if (!age) throw 'Umur tidak boleh kosong (Angka)'
  age = parseInt(age)
  if (age > 60) throw '*Ets Mau Dapet Pap TT Kan? Kalau Mau Daftar Yang Benar Umurnya*'
  if (age < 6) throw '*Ets Mau Dapet Pap TT Kan? Kalau Mau Daftar Yang Benar Umurnya*'
...
*Jika Kamu Butuh SN, Ketik ${usedPrefix}ceksn*

${chatnye}
`.trim())
} else m.reply(`Kamu Sudah *TERDAFTAR*\nMohon Tunggu ${waktuh} Untuk Bisa *DAFTAR* Kembali...`)
}
