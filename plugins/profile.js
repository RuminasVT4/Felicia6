...
let handler = async (m, { conn, text, usedPrefix, command }) => {
  ...
  if(isNaN(text)) {
   ...
   if(!text && !m.quoted) return conn.reply(m.chat, `*CARA PAKAI*\n\n• Tag User: *${usedPrefix}profile @Tag*\n• Pakai Nomor: *${usedPrefix}profile 6289654360447*\n\nCek Profile Sendiri? Reply Pesanmu Dengan Perintah #profile`, m)
    if(isNaN(number)) return conn.reply(m.chat, `*CARA PAKAI*\n\n• Tag User: *${usedPrefix}profile @Tag*\n• Pakai Nomor: *${usedPrefix}profile 6289654360447*\n\nCek Profile Sendiri? Reply Pesanmu Dengan Perintah #profile`, m)
    if(number.length > 15) return conn.reply(m.chat, `*CARA PAKAI*\n\n• Tag User: *${usedPrefix}profile @Tag*\n• Pakai Nomor: *${usedPrefix}profile 6289654360447*\n\nCek Profile Sendiri? Reply Pesanmu Dengan Perintah #profile`, m)
...
• Kadaluarsa Premium: ${(premiumDate - now) > 1 ? msToDate(premiumDate - now) : '*Bukan Premium*'}${lastclaim > 0 ? '\n• Last Claim: ' + new Date(lastclaim) : ''}
`.trim()
